...
void simulate(long long steps) {
    ...
    std::vector<std::queue<long long> > next_in_line;
    ...
    next_in_line.resize(K);
    idx.resize(K);

    for (long long i = 0; i < K; ++ i) idx[i] = i;
    ...
    for (long long step = 0, n_arrival = 0; step < steps;) {
        ...
        switch (which_e) {
            case 0:
                assert((n_arrival - step) == Q[0]+Q[1]+Q[2]+num_running[0]+num_running[1]+num_running[2]);
                ...
                if (!valid_schedule(num_running)) {
                    num_running[which_arrival] --;
                    Q[which_arrival] ++;
                    next_in_line[which_arrival].push(n_arrival);
            ...
            case 1:
                ...
                std::sort(idx.begin(), idx.end(), [&](const int &a, const int &b) {
                    if (!Q[b]) return true;
                    if (!Q[a]) return false;
                    return next_in_line[a].front() < next_in_line[b].front();
                });
                for (long long i = 0; i < K; ++ i) {
                    long long which_arrival = idx[i];
                    while (Q[which_arrival]) {
                        num_running[which_arrival] ++;
                        if (valid_schedule(num_running)) {
                            Q[which_arrival] --;
                            next_in_line[which_arrival].pop();
                        } else {
                            num_running[which_arrival]--;
                            break;
                        }
