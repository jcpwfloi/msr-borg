...
long long msr_state = 0;

std::vector<long long> Q;
std::vector<double> sum_Q;
std::vector<long long> samples;

std::vector<std::vector<double> > dtmc;
std::vector<double> dtmc_row_rate;
...
void simulate(long long steps) {
    for (int i = 0; i < n; ++ i)
        dtmc_row_rate[i] = std::reduce(dtmc[i].begin(), dtmc[i].end());
    ...
    for (long long step = 0; step < steps;) {
        ...
        element_wise_min(num_running, num[msr_state], Q);
        multiply(rates, mu, num_running);
        //rates = mu * num_running;
        double total_departure_rate = std::reduce(rates.begin(), rates.end());
        p = {total_arrival_rate, dtmc_row_rate[msr_state], total_departure_rate};
        next_p.set(p);
        which_e = next_p.get();
        switch (which_e) {
            case 0:
                ...
                if (step > 0.2 * steps) {
                    sum_Q[which_arrival] += Q[which_arrival];
...
int main() {
    ...
    dtmc.resize(n, std::vector<double>(n, 0.0));
    dtmc_row_rate.resize(n, 0.0);
