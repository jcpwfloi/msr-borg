...
bool valid(const vector<size_t> u) {
    ...
    return sum.first <= (1.0 + 1e-8) && sum.second <= (1.0 + 1e-8);
}
...
bool valid_type(size_t some_type) {
    ...
    return resource_in_use.first + type_resources[some_type].first <= (1.0 + 1e-8) && 
           resource_in_use.second + type_resources[some_type].second <= (1.0 + 1e-8);
}
